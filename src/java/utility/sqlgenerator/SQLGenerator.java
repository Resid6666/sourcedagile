...
import static utility.sqlgenerator.EntityManager.getEntityFields;
...
public class SQLGenerator {
    ...
    public static String insertGenerator(String entityName, Carrier crin, String[] methodNames) throws QException {

        String retQuery = INSERT_INTO + SPACE + entityName + SPACE + OPEN_BRACKET
                + getTableFieldNameFromListCore(methodNames) + CLOSE_BRACKET
...
//    }
    ...
    public static String updateGenerator(String entityName, Carrier crin, String[] methodNames, ArrayList valueList) throws QException {

        String updateSql = UPDATE + SPACE + entityName + SPACE + SET + SPACE
                + generateSetPartOfUpdateSql(crin, methodNames, valueList);
        String wPart = generateWherePartOfUpdate(crin, crin.get("id").trim(), valueList);
        if (wPart.equals("")) {
            return "";
        }
        updateSql += wPart;
...
//			if (!vl.trim().isEmpty()){
            setPart += seperateTableFieldNameWithUnderscore(methodName.substring("GET".length(), methodName.length()), databaseNumber).toUpperCase(Locale.ENGLISH) + EQUAL + "?" + SPACE + SEPERATOR_COMMA + SPACE;
//			}
        }
        return setPart.substring(1, setPart.length() - SEPERATOR_COMMA.length() - 1);
    }

    private static String generateSetPartOfUpdateSql(Carrier crin, String[] methodNames, ArrayList valueList) throws QException {
        String setPart = SPACE;
//        String[] methodNames = getAllGetMethodNames(coreEnt);
        for (String methodName : methodNames) {
            if (!crin.isKeyExist(methodName)) {
                continue;
            }

            if (methodName.trim().toLowerCase(Locale.ENGLISH).equals("id")) {
                continue;
            }

            valueList.add(crin.getValue(methodName));
            setPart += seperateTableFieldNameWithUnderscore(methodName).toUpperCase(Locale.ENGLISH) + EQUAL + "?" + SPACE + SEPERATOR_COMMA + SPACE;

        }
        return setPart.substring(1, setPart.length() - SEPERATOR_COMMA.length() - 1);
    }

    public static String generateSelectFieldPartOfSelectSql(Carrier crin, String[] methods) throws QException, Exception {
        String st = "";
        if (crin.hasSelectedField()) {
            st = selectFieldNameGeneratorByCustom(crin);
        } else if (crin.hasDistinctField()) {
            st = selectFieldNameGeneratorByDistinctFileds(crin);
...
//            st = selectFieldNameGeneratorByIncludedFileds(coreEnt, addEntityNameToAS);
    ...
    public static String generateSelectFieldPartOfSelectSql(CoreEntity coreEnt, String databaseNumber, boolean addEntityNameToAS) {
        String st = "";
        if (coreEnt.hasField()) {
            st = selectFieldNameGeneratorByCustom(coreEnt, addEntityNameToAS);
        } else if (coreEnt.hasDistinctFields()) {
...
//            st = selectFieldNameGeneratorByIncludedFileds(coreEnt, addEntityNameToAS);
    ...
    public static String selectFieldNameGeneratorByDistinctFileds(Carrier crin) throws QException {
        String st = SPACE + "   ";
        String[] arg = crin.getDistinctField();
        for (String arg1 : arg) {
            String fname = seperateTableFieldNameWithUnderscore(arg1).toUpperCase(Locale.ENGLISH);
            st += fname;
    ...
    private static String selectFieldNameGeneratorByMethodnames(Carrier crin, String[] methodNames) throws QException, Exception {
        String st = SPACE;
        for (String methodName : methodNames) {
            if (methodName.trim().length() == 0) {
                continue;
            }
            String fname = methodName;
            st += fname;
            st += SPACE + SEPERATOR_COMMA + SPACE;
//            }
//			}
...
//            if (!coreEnt.hasExcludedFieldName(fname)) {
            String fname1 = seperateTableFieldNameWithUnderscore(fname).trim().toUpperCase(Locale.ENGLISH);
            if (addEntityNameToAS) {
                st += tablename + "." + fname1;
...
//			}
    ...
    private static String selectFieldNameGeneratorByCustom(Carrier crin) throws QException {
        String st = SPACE;
        Object[] fields = crin.getSelectedField();
        for (Object f : fields) {
    ...
    private static String selectFieldNameGeneratorByCustom(CoreEntity coreEnt, boolean addEntityNameToAS) {
        String tablename = coreEnt.toDBTableName();
        String st = SPACE;
        Object[] fields = coreEnt.selectFieldList();
        for (Object f : fields) {
            String fname = f.toString();
//            fname = lowerFirstLetter(fname).trim();
//            if (!coreEnt.hasExcludedFieldName(fname)) {
...
//			}
    ...
    public static String getTableNameBasedOnEntity(CoreEntity core) {
        ...
        tableName = seperateTableFieldNameWithUnderscore(tableName).toUpperCase(Locale.ENGLISH);
...
//UNDERSCORE + capitalizeFirstLetter(databaseNumber));
    ...
    static String lowerFirstLetter(String arg) {
        arg = arg.substring(0, 1).toLowerCase(Locale.ENGLISH) + arg.substring(1, arg.length());
        return arg;
    }
    ...
    public static String seperateTableFieldNameWithUnderscore(String arg) {
        ...
        for (int i = 1; i < arg.length(); i++) {
            ...
            if (isNumeric(lt) && (!isNumeric(lastChar)) && (!lastChar.equals(UNDERSCORE))) {
                ...
                lt = lt;
            } else if (lt.toUpperCase(Locale.ENGLISH).equals(lt)) {
                lt = UNDERSCORE + lt;
            }
...
//        String propKeyName = HAS_UNDERSCORE_IN_FIELD_NAME + "." + databaseNumber.toLowerCase(Locale.ENGLISH);
...
//        }
    ...
    static String getTableFieldNameFromListCore(String arg[]) {
        String tbNames = "";
        /*first element of arg[] must contain a number of getMethods
         * */
        for (String arg1 : arg) {
            tbNames = tbNames + seperateTableFieldNameWithUnderscore(arg1) + SEPERATOR_COMMA;
        }
        tbNames = tbNames.toUpperCase(Locale.ENGLISH);
...
//        }
    ...
    public static String generateWherePartOfSelect(Carrier crin, String[] methodNames, String[] values, ArrayList valueArr) throws QException, Exception {

        String whereCondition = SPACE;
        String tablename = crin.getEntityName();
        ...
        try {
            for (int i = 0; i < methodNames.length; i++) {
                if (!values[i].trim().isEmpty()) {
                    ...
                    singleClause = getSingleClausOfWherePartOfSelect(tablename, key, val, valueArr);
    ...
    public static String generateWherePartOfSelect(CoreEntity coreEnt, String databaseNumber, String[] methodNames, String[] values, ArrayList valueArr,
        ...
        String whereCondition = SPACE;
        String tablename = coreEnt.toDBTableName();
        String sumByField = coreEnt.selectSumBy().trim().equals("") ? "" : seperateTableFieldNameWithUnderscore(coreEnt.selectSumBy()).toUpperCase(Locale.ENGLISH);;
        try {
            for (int i = 0; i < methodNames.length; i++) {
//                String vl = executeMethod(coreEnt, methodNames[i]).toString();
                if (!values[i].trim().isEmpty()) {
...
//            operation = "LOWER(" + modifyKey(addEntityNameToAS, tablename, key.trim().toLowerCase(Locale.ENGLISH)) + ")" + SPACE + COMMAND_LK;
            operation = modifyKey(addEntityNameToAS, tablename, key.trim().toLowerCase(Locale.ENGLISH)) + SPACE + COMMAND_LK;
    ...
    public static String generateWherePartOfUpdate(Carrier crin, String value, ArrayList valueArr) {
    ...
    public static String selectSortPartOfSelect(Carrier crin) throws QException {
        String s = "";
        String[] st = crin.getSortByField();

        for (String st1 : st) {
            if (st1.trim().length() > 0) {
                String sby = crin.getSortByAsc() ? " ASC " : " DESC ";
                String t = seperateTableFieldNameWithUnderscore(st1).toUpperCase(Locale.ENGLISH) + sby;
...
//        return updateStr;
    ...
    private static String generateMainSqlQueryBasedOnCarrier(String tableName, Carrier qc) throws Exception {
        ...
        for (int i = 0; i < cnt; i++) {
            query += qc.getValue(tableName, i, FIELD_SQL.toLowerCase(Locale.ENGLISH)).toString().trim();
        }
        return query;
    }
    ...
    private static String generateWherePartOfSelectMultipeSql(CoreEntity coreEnt) throws QException {
        ...
        for (int i = 0; i < methodNames.length; i++) {
            try {
                String fieldName = seperateTableFieldNameWithUnderscore(methodNames[i].substring("GET".length(), methodNames[i].length())).toUpperCase(Locale.ENGLISH);
    ...
    private static String generateWherePartOfUpdateSql(CoreEntity coreEnt) throws QException {
        ...
        for (String methodName : methodNames) {
            try {
                String fieldName = seperateTableFieldNameWithUnderscore(methodName.substring("GET".length(), methodName.length())).toUpperCase(Locale.ENGLISH);
