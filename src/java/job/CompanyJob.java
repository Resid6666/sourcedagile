...
//                System.out.println("ok 1");
                PreparedStatement pst = conn.prepareStatement("select lower(table_name) "
                        + "table_name from backlog.cr_user_tables where type='t'");
...
//                System.out.println("ok 2");
                ...
                pst = conn.prepareStatement("SELECT ut.table_script "
                        + "FROM backlog.cr_user_tables ut WHERE ut.type='v' ORDER BY SEQNUM");
                ...
                while (rs.next()) {
                    ...
                    for (String tableName : tableList) {
                        stmt.executeUpdate("CREATE TABLE " + companyDb + "." + tableName + " LIKE backlog." + tableName);
...
//                        System.out.println("view created: " + vs);
                    ...
                    psu = conn.prepareStatement("INSERT INTO " + companyDb + ".cr_user SELECT * from backlog.cr_user WHERE id=?");
                    psu.setString(1, userId);
                    psu.executeUpdate();

                    psu = conn.prepareStatement("DELETE FROM backlog.cr_user WHERE id=?");
                    psu.setString(1, userId);
                    psu.executeUpdate();
                }*/
                    psu = conn.prepareStatement("UPDATE backlog.cr_company SET status=? WHERE id=? AND status=?");
